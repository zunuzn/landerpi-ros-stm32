...
class BodyControlNode(Node):
    def __init__(self, name):
        ...
        set_servo_position(self.joints_pub, 1, ((10, 200), (5, 500), (4, 420), (3, 100), (2, 700), (1, 500)))
                ...
                if self.can_control:  # 可以控制则进入姿态检测部分(if control function can be implemented, move to posture detection part)
                    ...
                    if len(self.left_hand_count) == 4:
                        ...
                        if self.move_status[0]:
                            ...
                            if 'Acker' in  self.machine_type:
                                twist.angular.z = -1.0
                                self.get_logger().info('\033[1;32m left \033[0m')
                                threading.Thread(target=self.move, args=(twist, 4.5)).start()
                            ...
                            self.get_logger().info('\033[1;32m right \033[0m')
                            self.move_finish = False
                            self.last_status = 2
                            if 'Acker' in  self.machine_type:
                                twist.angular.z = -1.0
                            ...
                            if 'Acker' in self.machine_type:
                                self.publish_servo_state([3], [1500])
                                time.sleep(0.03)
                            self.move_finish = False
                            self.last_status = 3
                            twist.linear.x = 0.3
                            ...
                            self.get_logger().info('\033[1;32m b \033[0m')
                            if 'Acker' in self.machine_type:
                                self.publish_servo_state([3], [1500])
                                time.sleep(0.03)
                            self.move_finish = False
