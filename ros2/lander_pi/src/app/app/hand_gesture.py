...
class HandGestureControlNode(Node):
    ...
    def enter_srv_callback(self, request, response):
        ...
        set_servo_position(self.joints_pub, 1.5, ((10, 200), (5, 500), (4, 350), (3, 0), (2, 750), (1, 500)))
        response.success = True
        ...
        if len(msg.points) >= 5:
            ...
            if 0 <= angle < 30:
                if sum(left_and_right) > 0:
                    if 'Acker' in self.machine_type:
                        twist.angular.z = 1.0
                        self.publish_servo_state([3], [1179])  # 左转(turn left)
                    ...
                    if 'Acker' in self.machine_type:
                        twist.angular.z = -1.0
                        self.publish_servo_state([3], [1821])  # 右转(turn right)
                        threading.Timer(1.0, self.publish_servo_state, [[3], [1500]]).start()  # 1秒后停止(stop after 1 second)
                    elif 'Mecanum' in self.machine_type:
                        twist.linear.y = float(-self.linear_speed)
                    elif 'Tank' in self.machine_type:
