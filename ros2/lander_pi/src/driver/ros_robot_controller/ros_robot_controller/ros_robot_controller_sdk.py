...
class Board:
    ...
    def __init__(self, device="/dev/rrc", baudrate=1000000, timeout=10):
        ...
        self.encoder_queue = queue.Queue(maxsize=1)
        ...
        self.parsers = {
            ...
            PacketFunction.PACKET_FUNC_MOTOR: self.packet_report_motor_encoder,
    ...
    def packet_report_motor_encoder(self, data):
        if len(data) != 49 or data[0] != 0x06:
            return
        try:
            self.encoder_queue.put_nowait(data[1:])
        except queue.Full:
            pass

    def get_encoder_state(self):
        try:
            return struct.unpack('<4q4f', self.encoder_queue.get(block=False))
        except queue.Empty:
            return None
...
if __name__ == "__main__":
    ...
    for j in range(5):
        for i in range(2):
           ...
           time.sleep(1)
        print('moving!!!!')
        board.set_motor_speed([[1, 1.0], [2, -0.6], [3, 0], [4, -0.6]])
        time.sleep(5)
        print('STOP!!!!')
        board.set_motor_speed([[1, 1.0], [2, 0.0], [3, 0], [4, 0.0]])
        time.sleep(3)
        print(j)
